...
def chatting():
    ...
    full_prompt = f""" You are an AI assistant that can call tools if needed. 
    keep in mind that todays date is {date.today()} and tomorrow is {date.today() + timedelta(days=1)}, 
    you can add or subtract days from today to come up with the date Additionally, 
    i have given all of the information regarding all of the journal entries and their information, 
    so you can access them via: {allJournalEntries} the current journal entry the user is looking at is given by the number {currentEntry}. 
    match the number here to the ID of allJournalEntries to find the current journal entry the user is on. 
    However, if currentEntry is 0, then the user is not inside of a journal entry. 
    ...
    if the user is asking anything in the form of a question IT IS NOT A TOOL. NOT ALL RESPONSES REQUIRE THESE TOOLS. 
    if the question can be answered without a tool, answer without a tool first.
    if a user asks to change the entry text or the entry rating and currentEntry is 0, then do not call on any tools and specify that 
